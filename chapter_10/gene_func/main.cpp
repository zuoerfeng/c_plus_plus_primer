...
//测试find返回的是迭代器类型，通过返回值可以对迭代器进行解引用
void findCount(void)
{
    int val = 23;
    vector<int> ivec{12, 32, 23, 34, 45, 56, 67, 78, 89, 910};
//    string val("what the fvck design pattern");
    auto result = find(ivec.cbegin(), ivec.cend(), val); // 返回目标val相等的迭代器
    // 没找到就返回， 第二个参数 cend
    if (result == ivec.cend())
        cout << "not found " << endl;
    else
...
void readOnlyAlgorithm(void)
{
    ...
    // ivec 至少得 len >= svec
    vector<string> snvec{"what", "the", "fuck", "design", "pattern"};
    bool a = equal(svec.cbegin(), svec.cend(), snvec.cbegin());
    cout << a << endl;
}

//测试写算法，只要涉及赋值、复制，插入，替换操作
void writeAlgorithm(void)
{
    vector<int> ivec{27, 210, 83, 12, 47, 83, 109, 83};
    //赋值算法
...
//    [capture list](params list) -> return type {func body}
	...
	auto f_1_iter = find_if(words.cbegin(), words.cend(), f4);
...
//=4= 如果在异步 lambda 中通过引用捕获本地变量，该本地变量将很可能在 lambda 运行时消失，从而导致运行时访问冲突。
    ...
    auto f7 = [v1]() mutable {
        cout << v1 << endl;
        ++v1; // 不加mutable v1就是read-only variable,只能读，不能写
        cout << v1 << endl;
        cout << &v1 << endl;
        return v1;
    };
    ...
    using namespace std::placeholders;    //_f前面下划线的都在这个命名空间里面
